...
export function RecordSubscriptionPaymentSection({
  ...
  const refreshInvoices = useCallback(async () => {
    const res = await fetch(`/api/subscription-invoices/outstanding?${pageQueryString(page, pageSize, searchInput)}`, {
      cache: "no-store",
    });
    if (!res.ok) {
    ...
    if (data.items.length === 0 && data.page > 1) {
      const res2 = await fetch(
        `/api/subscription-invoices/outstanding?${pageQueryString(data.page - 1, pageSize, searchInput)}`,
        { cache: "no-store" },
      );
      if (res2.ok) {
  ...
  async function goToPage(nextPage: number) {
    const res = await fetch(`/api/subscription-invoices/outstanding?${pageQueryString(nextPage, pageSize, searchInput)}`, {
      cache: "no-store",
    });
    if (!res.ok) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <TableSearchBar
        id="record-subscription-payments-search"
        value={searchInput}
        onChange={setSearchInput}
        placeholder="Account, patient, plan…"
      />
