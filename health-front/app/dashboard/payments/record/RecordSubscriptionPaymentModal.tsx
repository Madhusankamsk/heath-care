...
type Props = {
  open: boolean;
  invoice: OutstandingSubscriptionInvoiceRow | null;
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(
        `/api/subscription-invoices/${encodeURIComponent(invoice.id)}/payments`,
        {
          method: "POST",
        ...
        <>
          <div className="mb-4 rounded-xl border border-[var(--border)] bg-[var(--surface-2)]/50 px-3 py-2 text-xs text-[var(--text-secondary)]">
            <div className="grid gap-1 sm:grid-cols-2">
              <span>
                <span className="text-[var(--text-muted)]">Plan:</span> {invoice.planName}
              </span>
              <span>
              ...
              <span className="sm:col-span-2">
                ...
                <span className="font-medium text-[var(--text-primary)]">
                  {invoice.suggestedPaymentPurposeLabel}
                </span>
              </span>
            </div>
          </div>
